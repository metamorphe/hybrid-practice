...
class window.LEDPlacerBrush
  ...
  constructor: (paper) ->
    ...
    @tool.onMouseDown = (event) ->
      ...
      # NO SELECTION LOGIC
      if !event.modifiers.shift and !event.event.metaKey and [ 'NLED' ].indexOf(name) == -1
        sm.selection = []
        sm.update()
        return
      ...
      if event.event.metaKey
        ...
        return
      # TARGET MODE
      if scope.target_selection_mode or event.modifiers.shift
    ...
    @tool.onMouseMove = (event) ->
      ...
      # 	if(name == "NLED"){
      # 		if(event.event.metaKey){
      # 			$('#main-canvas').css('cursor', 'not-allowed');
      # 			return;
      # 		}else{
      ...
      # }
      # if(event.event.metaKey){
      # 	$('#main-canvas').css('cursor', 'copy');
      # 	if (hitResult){
      # 		path = hitResult.item;
      # 		name = Artwork.getPrefix(path);
      # 		if(name == "NLED") $('#main-canvas').css('cursor', 'alias');
      # 	}
      # }else{
    ...
    @tool.onMouseDrag = (event) ->
      ...
      _.each drags, (drag) ->
        ...
        cdiffs = _.flatten([
          CanvasUtil.queryPrefix('DIF')
          CanvasUtil.queryPrefix('DDS')
        ])
        contained_by = _.filter(cdiffs, (diff) ->
          diff.contains drag.position
        )
        main_container = _.min(contained_by, (diff) ->
          diff.position.getDistance drag.position
        )
        if drag.forceTarget
          ...
          if !forced_diff.contains(drag.position)
            ...
            diffusers_in_force_target = _.filter(cdiffs, (diff) ->
              forced_diff.id != diff.id and !diff.contains(forced_diff.position) and forced_diff.contains(diff.position)
            )
            # console.log("DIFF IN FT", forced_diff.id, _.map(diffusers_in_force_target, function(d){return d.id}));
            in_another_diffuser = _.compact(_.map(diffusers_in_force_target, (diff) ->
              diff.contains drag.position
            )).length > 0
          ...
          if diffs.length == 0
            ...
            if ixts.length > 0
              closestIxT = _.min(ixts, (ixt) ->
                ixt.point.getDistance r.position
              )

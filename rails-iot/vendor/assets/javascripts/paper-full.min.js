...
paper.loadCustomLibraries = function(){
	...
	paper.project.computeHull = function(pts){
		// in point form; must convert to [[x,y]]
		// console.log("COMPUTING HULL", pts.length, pts)
		var pts = _.map(pts, function(pt){
			return [pt.x, pt.y];
		});

		pts = hull(pts, 50);

		var pts = _.map(pts, function(p){
	...
	paper.project.computeOMBB = function(hull){
		var degree_of_rotation = _.range(0,360, 1);
		var cost_table = _.map(degree_of_rotation, function(angle,i,arr){
			hull.rotation = angle;
			var pos_ombb = new paper.Path.Rectangle(hull.bounds.clone());
			var cost = paper.project.computeOMBBCost(hull, pos_ombb);
			return {
				angle: angle,
				cost: cost
			};
	...
	paper.Path.prototype.calculateOMBB = function(){
		var scope = this;
		pts = _.range(0, this.length, 0.5);
		pts = _.map(pts, function(pt){ return scope.getPointAt(pt); })

		this.hull = paper.project.computeHull(pts);
		var angle = paper.project.computeOMBB(this.hull);	
		var bb = this.hull.clone();
		console.log("ANGLE", angle)
		bb.rotation = - angle;
		var ombb = new paper.Path.Rectangle({
			rectangle: bb.bounds
		})
		bb.remove();
		ombb.rotation = angle;
		...
		this.ombb.visible = false;
		return this;
	}

	paper.Path.prototype.getMagnets = function(ombb){
		ombb.rotation = 0;
		this.magnets = paper.project.computeMagnets(ombb);
		
		this.magnets.west  = this.magnets[0];
		this.magnets.north = this.magnets[1];
		this.magnets.east  = this.magnets[2];
		this.magnets.south = this.magnets[3];
		return this;
	}
	paper.Path.prototype.centroid = function(){
        calc_centroid = _.reduce(this.segments, function(memo, seg) {
            return memo.add(new paper.Point(seg.point.x, seg.point.y));
        }, new paper.Point(0, 0));

        return calc_centroid.divide(this.segments.length);
	}
    ...
    paper.CompoundPath.prototype.expand = function(o){
	    ...
	    _.each(this.children, function(p, i){
			p.expand(o)
	    })
	    return cp
    }
    paper.Group.prototype.expand = function(o){ 	
    	_.each(this.children, function(p, i){
    		p.expand(o)
    	})
    }

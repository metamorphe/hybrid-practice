...
paper.loadCustomLibraries = function(){
	...
	paper.project.computeConvex = function(pts){
		// in point form; must convert to [[x,y]]
		var points = _.map(pts, function(pt){
			return [pt.x, pt.y];
		});
		// convex points
		var convex_points = hull(points, 50);
		var convex_points = _.map(convex_points, function(p){
			return new paper.Point(p[0],p[1]);
		});
		var convex = new paper.Path({
			segments: convex_points,
		});
		return convex;
	}

	paper.project.computeOMBB = function(pts){
		var convex = paper.project.computeConvex(pts);
		var degree_of_rotation = _.range(0,360);
		var cost_table = _.map(degree_of_rotation, function(angle,i,arr){
			convex.rotation = angle;
			var pos_ombb = new paper.Path.Rectangle(convex.bounds.clone());
			var cost = paper.project.computeOMBBCost(convex,pos_ombb);
			return {
				angle: angle,
				cost: cost
			};
		...
		OMBB = new paper.Path.Rectangle(convex.bounds.clone());
		OMBB.visible = false;
	...
	paper.project.computeMagnets = function(ombb){
		...
		// adds the NESW terminals
		var magnets = _.map(lines, function(line){
			return line.getPointAt(line.length/2);
		});
		// adds corner terminals
		_.each(ombb.segments, function(seg){
			magnets.push(seg.point);
		})

		return magnets;
	}
	paper.project.computeTerminal = function(path, orientation){
		// path is the path that is to be labeled
		path.calculateOMBB();
		path.setMagnets(path.ombb);

		// Create vector from center to center + 1000
		var path_center = path.ombb.bounds.center.clone();
		path_center.length = 1000;
		path_center.angle = orientation;

		// Calculate the intersections between the path and the vector
		path_vector = new paper.Path([path.ombb.bounds.center, path_center])
		
		// Determine the closest magnet to the intersection found
		path_itx = path.ombb.getIntersections(path_vector);
		var magnets = _.map(path.magnets, function(value, key){ return {name: key, point: value }});
		var path_terminal = _.min(magnets, function(m){
			return m.point.getDistance(path_itx[0].point);
		});
		// Remove vector from canvas
		path_vector.remove();
		return path_terminal;
	}

	paper.Path.prototype.calculateOMBB = function(){
		var pts = [];
		for (var i = 0; i < this.length; i++){
			pts.push(this.getPointAt(i));
		}
		this.ombb = paper.project.computeOMBB(pts);	
	}
	paper.Path.prototype.setMagnets = function(ombb){
		ombb.rotation = 0;
		magnets = paper.project.computeMagnets(ombb);
		this.magnets = {}
		this.magnets.west        =  magnets[0];
		this.magnets.north       =  magnets[1];
		this.magnets.east        =  magnets[2];
		this.magnets.south       =  magnets[3];
		// this.magnets.bottomLeft  =  magnets[4];
		// this.magnets.topLeft     =  magnets[5];
		// this.magnets.topRight    =  magnets[6];
		// this.magnets.bottomRight =  magnets[7];
	}
		paper.Group.prototype.setMagnets = function(){
		magnets = paper.project.computeMagnets(new paper.Path.Rectangle(this.bounds));
		this.magnets = {}
		this.magnets.west        = magnets[0];
		this.magnets.north       = magnets[1];
		this.magnets.east        = magnets[2];
	...
	paper.Path.prototype.setTerminal = function(orientation){
		this.terminal = paper.project.computeTerminal(this, orientation);
	}

  paper.Path.prototype.expand = function(amount){
    var circles = [];
    var samples = _.range(0, this.length + 12, 12);
    var rect = _.map(samples, function(offset){
    	var x = path.getPointAt(offset).clone();
    	var n = path.getNormalAt(offset);
    	n.length = amount;
    	x = x.add(n);
